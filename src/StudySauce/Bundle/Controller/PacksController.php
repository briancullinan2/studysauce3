...
class PacksController extends Controller
{
    ...
    public function responsesAction(Request $request, User $user)
    {
        ...
        if (!empty($request->get('pack'))) {
            ...
            $packs = $user->getPacks()->filter(function (Pack $p) use ($user, $currentUser) {return !$p->getDeleted() && $p->getStatus() != 'UNPUBLISHED' && !empty($p->getStatus()) && in_array($user, self::getChildUsersForPack($p, $currentUser));});
