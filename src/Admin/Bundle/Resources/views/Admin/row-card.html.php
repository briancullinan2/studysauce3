...
print (!empty($classes) ? $classes : ''); ?>">
    <?php print $view->render('AdminBundle:Admin:cells.html.php', ['entity' => $card, 'tables' => $tables, 'table' => $table, 'allGroups' => $allGroups, 'searchRequest' => $searchRequest]); ?>
    <label class="checkbox"><input type="checkbox" name="selected"/><i></i></label>
</div>
<?php if (isset($expandable[$table])) { ?>
    <div class="expandable <?php
    print (!empty($classes) ? $classes : ''); ?>">
    <?php print $view->render('AdminBundle:Admin:cells.html.php', ['entity' => $card, 'tables' => $expandable, 'table' => $table, 'allGroups' => $allGroups, 'searchRequest' => $searchRequest]); ?>
    </div><?php
}