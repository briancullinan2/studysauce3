...
<div class="highlighted-link">
    ...
    if(isset($searchRequest['pack-id']) && !empty($group = $searchRequest['pack-id'])) { ?>
        <a href="#general-dialog" data-dialog="Are you sure you would like to remove the group &ldquo;<?php print $ss_group->getName(); ?>&rdquo; from the pack?" class="remove-icon" data-action="<?php print $view['router']->generate('save_group', ['groupId' => $searchRequest['pack-id'], 'packId' => $ss_group->getId(), 'groups' => [['remove' => 'true', 'id' => $searchRequest['pack-id']]]]); ?>" data-target="#general-dialog" data-toggle="modal">&nbsp;</a>
    <?php }
    else { ?>
